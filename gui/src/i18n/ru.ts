...
// Strings with {cmd} render a <code> chip via <Trans>; {var} are plain interpolations.
  ...
  "dash.codexAutoStartHint": "Когда параметр включён, обёртка (shim) Codex выполняет ocx ensure перед каждым запуском Codex CLI/App. Отключите его, чтобы оставить Codex нетронутым.",
  ...
  // models
  "models.subtitle": "Управляйте тем, какие модели видит Codex — нативные GPT (сквозной проброс) и модели маршрутизируемых провайдеров, сгруппированные по провайдеру (нажмите на заголовок, чтобы свернуть группу). Отключённые модели скрываются из каталога и селектора моделей. Изменения применяются на следующем ходе Codex — opencodex сбрасывает 5-минутный кэш моделей Codex, поэтому перезапуск не требуется.",
  ...
  "models.v2Applied": "multi_agent_v2 обновлён — применяется к новым сессиям (перезапустите приложение Codex, чтобы обновить селектор моделей)",
  ...
  "storage.subtitle": "Обзор того, чем занято место в CODEX_HOME, — только для чтения. Эта страница ничего не удаляет.",
  ...
  "pws.workspaceToggle": "Рабочая область",
  "pws.classicToggle": "Классический вид",
  ...
  "pws.modelsNeedsReauth": "Для обнаружения моделей необходимо повторно войти в аккаунт. Пока отображаются настроенные модели.",
  "pws.modelsConfiguredFallback": "Отображаются настроенные модели (автоматическое обнаружение недоступно).",
  "pws.modelsTruncated": "Показаны первые {shown} из {total} моделей. Примените фильтр для уточнения списка.",
  "pws.retry": "Повторить",
  ...
  "modal.baseUrlPlaceholderError": "Базовый URL содержит неразрешённый {placeholder}. Замените его вашим фактическим значением.",
  "modal.baseUrlPlaceholderHint": "Замените {placeholder} в базовом URL на ваш фактический Account ID перед добавлением.",
  ...
  "codexAuth.nextSession": "СЛЕД. СЕССИЯ",
  ...
  "codexAuth.switchDesc": "Изменение вступит в силу со следующей сессии Codex. Текущая сессия останется на прежнем аккаунте.",
  "codexAuth.cacheWarning": "Кэш промптов сбрасывается при смене аккаунта. Новая сессия начнётся с пустым кэшем.",
  "codexAuth.setAsNext": "Назначить на следующую сессию",
  "codexAuth.cancel": "Отмена",
  "codexAuth.switchBack": "Вернуться на основной аккаунт?",
  "codexAuth.switchBackDesc": "Следующая сессия Codex снова будет использовать ваш аккаунт входа через приложение.",
  "codexAuth.autoSwitch": "Автопереключение при 80%",
  "codexAuth.autoSwitchDesc": "Новые сессии переключаются, когда соответствующее окно квоты достигает порога; тарифы Go/Free используют только окно 30 дней",
  "codexAuth.switched": "{email} будет использоваться со следующей сессии",
  "codexAuth.loadFailed": "Не удалось загрузить аккаунты.",
  ...
  "codexAuth.copyLoginLink": "Копировать ссылку для входа",
  "codexAuth.loginLinkCopied": "Ссылка для входа скопирована",
  "codexAuth.loginLinkCopyFailed": "Не удалось скопировать ссылку для входа.",
  ...
  "api.endpointNote": "Совместимо с форматом OpenAI Responses API.",
  ...
  "cws.addSubtitle": "Создайте виртуальную модель, которая распределяет запросы между провайдерами. Клиенты запрашивают combo/<id>.",
  ...
  "cws.howBody": "Запросите у Codex модель combo/<id>. OpenCodex выбирает цель и переключается на следующую только при сбоях вышестоящего провайдера, допускающих повтор. Если доступных целей не осталось, запрос завершается ошибкой, а не переходит на глобальный провайдер по умолчанию.",
  ...
  "cws.field.defaultEffortHint": "Используется, только если клиент не указал уровень рассуждений. Значение клиента имеет приоритет, а каждая цель применяет собственную обработку возможностей.",
  "cws.targets": "Цели",
